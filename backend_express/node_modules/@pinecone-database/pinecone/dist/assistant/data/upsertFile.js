 ...
 */
const upsertFile = (assistantName, apiProvider, config) => {
    return async (options) => {
        ...
        let filesUrl = `${hostUrl}/files/${encodeURIComponent(assistantName)}/${encodeURIComponent(options.assistantFileId)}`;
        (0, requestPath_1.assertRequestPathIsAddressable)(filesUrl);
        if (options.multimodal !== undefined) {
            filesUrl += `?multimodal=${options.multimodal}`;
        }
        return (0, fileUpload_1.sendFileMultipart)('PUT', filesUrl, options, config);
    };
};
exports.upsertFile = upsertFile;
const validateUpsertFileOptions = (options) => {
    if (!options) {
        throw new errors_1.PineconeArgumentError('You must pass an object with required properties (`assistantFileId` and `path` or `file` + `fileName`) to upsert a file.');
    }
    if (!options.assistantFileId) {
        throw new errors_1.PineconeArgumentError('You must pass the `assistantFileId` of the file to upsert.');
    }
    if (!('path' in options) && !('file' in options)) {
        throw new errors_1.PineconeArgumentError('You must pass either `path` or `file` + `fileName` to upsert a file.');
    }
    ...
    if ('file' in options) {
        ...
        if (!options.fileName) {
            throw new errors_1.PineconeArgumentError('`fileName` is required when upserting via `file`.');
        }
    }
};
