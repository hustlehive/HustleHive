...
const mergeHeaders = (existing, additionalHeaders) => {
    const base = toHeaderRecord(existing);
    return { ...base, ...(0, additionalHeaders_1.withoutContentType)(additionalHeaders) };
};
const additionalHeadersMiddleware = (additionalHeaders) => ({
    pre: async ({ url, init }) => ({
        url,
 ...
 */
const createMiddlewareArray = (additionalHeaders) => {
    const headerMiddleware = additionalHeaders && Object.keys(additionalHeaders).length > 0
    ...
    return [
        {
            pre: async (context) => {
                (0, requestPath_1.assertRequestPathIsAddressable)(context.url);
            },
        },
        ...headerMiddleware,
        ...debugMiddleware,
