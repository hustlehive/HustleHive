...
exports.assertRequestPathIsAddressable = void 0;
 ...
 * @internal
 */
const assertRequestPathIsAddressable = (url) => {
    ...
    const destination = resolvedPath
        ...
        : '';
    throw new errors_1.PineconeArgumentError(`The request path "${requestedPath}" contains the segment "${dotSegment}", which URL resolution removes before the request is sent.${destination} ` +
        `Pass a resource identifier that is not "." or ".."; percent-encoding does not make either safe.`);
};
exports.assertRequestPathIsAddressable = assertRequestPathIsAddressable;
