...
exports.pollUntilIndexIsReady = pollUntilIndexIsReady;
 ...
 */
async function pollUntilIndexIsReady(describeFn, indexName, timeoutMs, pollIntervalMs = 5000) {
    ...
    while (timeoutMs === undefined || Date.now() - start < timeoutMs) {
        ...
        if (state && TERMINAL_FAILED_STATES.has(state)) {
            throw new errors_1.PineconeIndexInitializationFailedError(indexName);
        }
        if (state && TERMINAL_TERMINATED_STATES.has(state)) {
            throw new errors_1.PineconeIndexTerminatedError(indexName, state);
        }
        await new Promise((r) => setTimeout(r, pollIntervalMs));
    }
    throw new errors_1.PineconeTimeoutError(indexName, timeoutMs);
}
//# sourceMappingURL=pollUntilIndexIsReady.js.map