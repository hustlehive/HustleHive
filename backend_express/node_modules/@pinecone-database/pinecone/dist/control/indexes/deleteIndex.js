 ...
 *
 */
async function deleteIndex(api, name) {
    if (!name) {
        throw new errors_1.PineconeArgumentError('You must pass a non-empty string for `name` in order to delete an index.');
    }
    try {
        ...
        throw await (0, handling_1.handleApiError)(e, async (_, rawMessageText) => `Error deleting index ${name}: ${rawMessageText}`);
    }
}
